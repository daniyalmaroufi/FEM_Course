...
element(:,5)=b(:,5);     %belongs to the area in the element.
% element(:,6)=b(:,6);
...
force(:,3)=c(:,3);      %belongs to the amount of force in x-direcion or increase in temperature.
force(:,4)=c(:,4);      %belongs to the amount of force in y-direcion or increase in temperature.

d=dlmread(BCdfile,' ');
disp(:,1)=d(:,1);       %belongs to no. of node under constraint.
disp(:,2)=d(:,2);       %belongs to the displacement in the x/y-direction.
disp(:,3)=d(:,3);       %an identifier for direction of constraint(1=x, 2=y).



%calculate elements angle
angles=calc_angles(node,element);

%calculations of stiffness matrix
K=k_matrix_global(node,element,angles);

%calculations of stiffness matrix
% K=k_matrix_local(node,element);

s=K;

F=BCf(force,node,element,disp,K);
